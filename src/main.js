...
function approvalChart(approval) {
  if (!approval.series?.length) {
    return `<div class="approval-chart no-data-card">${noDataBadge('No verified approval polling connected')}<p class="method-note">${icon.alert} ${approval.methodology}</p></div>`;
  }
  const max = 65;
  const min = 25;
