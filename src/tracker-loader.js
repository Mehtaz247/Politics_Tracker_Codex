const TRACKER_PAGES = [
  ...
  ['/investigations.html', 'Leads'],
  ['/radar.html', 'Radar'],
  ['/briefing.html', 'Briefing'],
  ['/search.html', 'Search'],
  ['/topic.html', 'Dossiers'],
  ['/notebook.html', 'Notebook'],
  ['/accountability.html', 'Grid'],
  ['/network.html', 'Power Map'],
...
export function renderTrackerNavLinks(slug, currentPath = '', trackerLabel = '') {
  const links = TRACKER_PAGES.map(([path, label]) => {
  ...
  return `${links}${trackerLabel ? `<span class="updated">${escapeHtml(trackerLabel)}</span>` : ''}`;
}
